...
const CardComponent: React.FC<CardProps> = ({ post }) => {
  return (
    <section className="m-3 my-10 p-2 shadow-lg max-w-[350px]  rounded-2xl transition ease-in-out delay-75 hover:bg-slate-200 ">
      <span className={`text-[#160A06] block font-medium`}>{post.title}</span>
      <span className="text-slate-500">{currentDate()}</span>
      <p className="text-sm text-[#160A06] mt-2">{`${post.body.substring(
        0,
        100
      )}...`}</p>
      <div className="flex justify-end">
        <button
          className={`bg-[#FAD6B1] hover:text-[#fad0a6] hover:bg-[#160A06] transition ease-in-out delay-75 px-2 py-1 mt-4 rounded-lg text-[#160A06] text-center text-sm`}
        >
          ادامه دهید
        </button>
      </div>
